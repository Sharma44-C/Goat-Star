...
const messageHistor const axios = require("axios");
...
async function queryAI(prompt, sessionId) {
    try {
        const response = await axios.get(`https://kai-api-2.onrender.com/chat?query=${encodeURIComponent(prompt)}&uid=${sessionId}`, {
            timeout: 10000
        });
...
module.exports = {
    ...
    processMessage: async function({ api, event, messageText }) {
        ...
        try {
            ...
            const response = await queryAI(prompt, event.sessionId);
            if (response) {
